...
app.post('/join', (req, res) => {
  ...
  if (session && session.pass === req.body.joiner.joinerSessionPass) {
    ...
    res.json({
      success: true,
      payload: db.sessions[req.body.joiner.joinerSessionId],
    })
  } else if (session) {
    ...
    res.json({
      success: false,
      message: 'no session with id:' + req.body.joiner.joinerSessionId,
      payload: null,
    })
  }
})
...
app.post('/vote', (req, res) => {
  ...
  if (db.sessions[sessionId] && username in db.sessions[sessionId].users) {
    ...
    res.json({
      success: true,
      message: 'vote added for user ' + username,
      payload: { [username]: voteResponse },
    })
  } else {
